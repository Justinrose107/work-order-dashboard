...
const filterIds=['search','region','sub','country','status','owner','type','priority'];
let dbPromise=null;
function openDb(){
...
['region','sub','country','status','owner','type','priority','sort'].forEach(k=>$(k).onchange=()=>{if(k==='region'){ $('sub').value='';updateSub();}page=1;render();schedulePersist();});$('search').oninput=()=>{page=1;render();schedulePersist();};
...
['region','sub','country','status','owner','type','priority'].forEach(k=>options(k,[],'All'));render();restoreSaved();setInterval(render,60000);
